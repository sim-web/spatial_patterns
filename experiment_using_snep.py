   ...
   # Note: Maybe you don't need to use Parameter() if you don't have units
	param_ranges = {
		...
		'inh': 
			...
			'sigma_x':ParameterArray([0.5, 0.5, 0.06, 0.04, 0.2, 0.2]),
			'sigma_y':ParameterArray([0.06, 0.04, 0.5, 0.5, 0.5, 0.6]),
			# 'n':ParameterArray([1, 100]),
			# 'fields_per_synapse':ParameterArray([1]),
			# 'weight_overlap':ParameterArray([0.0, 0.2]),
			# 'sigma_noise':ParameterArray([0.1]),
	...
	params = {
		...
		'exc':
			...
			'init_weight':ParameterArray(1. * target_rate / n_exc),
		...
		'inh':
			...
			'n': n_inh,
			'fields_per_synapse': 1,
			'init_weight':ParameterArray(1. * target_rate / n_inh),
			'init_weight_spreading': 0.05,
			'init_weight_distribution': 'uniform',
			}
	}
